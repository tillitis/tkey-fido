...
/*
 * Try to import scalar (private key) from big-endian bytes
 *
 * in: priv: possible big-endian bytes representation of a scalar
 * out: pub (x, y) concatenated, fixed-width 256-bit big-endian integers
 *      return 0 if scalar could be imported, -1 otherwise
 */
int p256_keypair_from_bytes(uint8_t pub[64], uint8_t priv[32])
{
    uint32_t s[8], x[8], y[8];
    ...
    /* compute and output the associated public key */
    scalar_mult(x, y, p256_gx, p256_gy, s);
    point_to_bytes(pub, x, y);
    return P256_SUCCESS;
}
